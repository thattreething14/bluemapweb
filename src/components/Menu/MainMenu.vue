<template>
  <SideMenu :open="menu.isOpen"
    ...
    <div v-if="menu.currentPage().id === 'root'">
      ...
      <SimpleButton @action="menu.openPage('towns', () => $t('towns.title'))" :submenu="true">{{ $t("towns.button")}}</SimpleButton>
    ...
    <CustomMenu v-if="menu.currentPage().id === 'towns'"/>
...
import CustomMenu from "@/components/Menu/TownsMenu.vue";
...
export default {
  ...
  components: {CustomMenu, MapButton, MarkerSetMenu, SettingsMenu, SimpleButton, SideMenu},
  props: {
    menu: MainMenu
  },
  data() {
